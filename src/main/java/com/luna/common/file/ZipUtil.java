...
import com.luna.common.file.compress.*;
...
public class ZipUtil {
    ...
    public static void append(Path zipPath, Path appendFilePath, CopyOption... options) throws IOException {
        try (FileSystem zipFileSystem = FileSystemUtil.createZip(zipPath.toString())) {
            if (Files.isDirectory(appendFilePath)) {
                ...
                Files.copy(appendFilePath, zipFileSystem.getPath(FileTools.getName(appendFilePath)), options);
            }
    ...
    public static File zip(File srcFile, Charset charset) throws IOException {
        final File zipFile = new File(srcFile.getParentFile(), FileNameUtil.mainName(srcFile) + ".zip");
    ...
    public static File unzip(File zipFile, Charset charset) {
        final File destDir = new File(zipFile.getParentFile(), FileNameUtil.mainName(zipFile));
    ...
    public static File unzip(String zipFilePath, String outFileDir, Charset charset) {
        ...
        return unzip(new File(zipFilePath), new File(outFileDir), charset);
    }
    ...
    public static File unzip(ZipFile zipFile, File outFile) {
        if (outFile.exists() && outFile.isFile()) {
            throw new IllegalArgumentException(
                StrFormatter.format("Target path [{}] exist!", outFile.getAbsolutePath()));
        }
    ...
    public static byte[] unzipFileBytes(String zipFilePath, Charset charset, String name) throws IOException {
        return unzipFileBytes(new File(zipFilePath), charset, name);
    }
    ...
    public static byte[] gzip(String content, String charset) {
        return gzip(CharsetUtil.bytes(content, CharsetUtil.charset(charset)));
    }
    ...
    public static String unGzip(byte[] buf, String charset) {
        return CharsetUtil.str(unGzip(buf), CharsetUtil.charset(charset));
    }
    ...
    public static byte[] zlib(String content, String charset, int level) {
        return zlib(CharsetUtil.bytes(content, CharsetUtil.charset(charset)), level);
    }
    ...
    public static String unZlib(byte[] buf, String charset) {
        return CharsetUtil.str(unZlib(buf), CharsetUtil.charset(charset));
    }
    ...
    private static void validateFiles(File zipFile, File... srcFiles) throws IOException {
        ...
        for (File srcFile : srcFiles) {
            ...
            if (srcFile.isDirectory() && parentPath.startsWith(sunPath)) {
                String format =
                    StrFormatter.format("Zip file path [{}] must not be the child directory of [{}] !", zipFile.getPath(), srcFile.getPath());
                throw new IOException(format);
            }
        }
    }
