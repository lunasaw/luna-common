...
public class IoUtil extends NioUtil {
     ...
     * @since 5.5.8
     */
    public static BufferedReader getReader(BOMInputStream in) throws IOException {
        return getReader(in, CharsetUtil.charset(in.getBOMCharsetName()));
    }
    ...
    public static String read(InputStream in, Charset charset) throws IOException {
        return CharsetUtil.str(readBytes(in), charset);
    }
    ...
    public static <T> T readObj(InputStream in, Class<T> clazz) throws IOException, UtilException {
        try {
            return readObj(
                (in instanceof ValidateObjectInputStream) ? (ValidateObjectInputStream)in
    ...
    public static ByteArrayInputStream toStream(String content, Charset charset) {
        ...
        return toStream(CharsetUtil.bytes(content, charset));
    }
    ...
    public static void write(OutputStream out, Charset charset, boolean isCloseOut, Object... contents)
        ...
        try {
            ...
            for (Object content : contents) {
                if (content != null) {
                    osw.write(ConvertUtil.toStr(content, StrPoolConstant.EMPTY));
                }
            }
            osw.flush();
        } catch (IOException e) {
