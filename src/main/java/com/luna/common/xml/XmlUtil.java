...
public class XmlUtil {
    ...
    public static Document readXML(File file) {
        ...
        try {
            in = FileTools.getInputStream(file);
            return readXML(in);
        } finally {
            IoUtil.close(in);
        }
    ...
    public static Document readXML(String pathOrContent) {
        if (StringTools.startWith(pathOrContent, '<')) {
            return parseXml(pathOrContent);
        }
        return readXML(FileTools.file(pathOrContent));
    }
    ...
    public static Document parseXml(String xmlStr) {
        if (StringTools.isBlank(xmlStr)) {
            throw new IllegalArgumentException("XML content string is empty !");
        }
        xmlStr = cleanInvalid(xmlStr);
        return readXML(StringTools.getReader(xmlStr));
    }
    ...
    public static String toStr(Node doc, String charset, boolean isPretty, boolean omitXmlDeclaration) {
        final StringWriter writer = StringTools.getWriter();
        try {
    ...
    public static void toFile(Document doc, String path, String charsetName) {
        ...
        try {
            writer = FileTools.getWriter(FileTools.touch(path), CharsetUtil.charset(charsetName), false);
    ...
    public static void transform(Source source, Result result, String charset, int indent, boolean omitXmlDeclaration) {
        ...
        try {
            ...
            if (StringTools.isNotBlank(charset)) {
    ...
    public static DocumentBuilderFactory createDocumentBuilderFactory() {
        ...
        if (StringTools.isNotEmpty(defaultDocumentBuilderFactory)) {
    ...
    public static String cleanComment(String xmlContent) {
        ...
        return xmlContent.replaceAll(COMMENT_REGEX, StringTools.EMPTY);
    }
    ...
    public static List<Element> getElements(Element element, String tagName) {
        final NodeList nodeList = StringTools.isBlank(tagName) ? element.getChildNodes() : element.getElementsByTagName(tagName);
    ...
    public static void writeObjectAsXml(File dest, Object bean) {
        ...
        try {
            xmlenc = new XMLEncoder(FileTools.getOutputStream(dest));
            ...
            if (null != newValue) {
                if (null != value) {
                    if (value instanceof List) {
                        ...
                        result.put(childEle.getNodeName(), Lists.newArrayList(value, newValue));
                    }
                } else {
     ...
     * @since 5.1.2
     */
    public static String mapToXmlStr(Map<?, ?> data, String rootName, String namespace, String charset, boolean isPretty,
    ...
    private static Node appendText(Document doc, Node node, CharSequence text) {
        return node.appendChild(doc.createTextNode(StringTools.str(text)));
    }
